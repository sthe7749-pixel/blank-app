...
def once():
    ...
    def pnl_rows_long(entry, stop, tgts):
        rows=[]
        for t in tgts:
            pct = ((t-entry)/entry)*lev*100; usdt = pos*(pct/100)
    ...
    def pnl_rows_short(entry, stop, tgts):
        rows=[]
        for t in tgts:
            pct = ((entry-t)/entry)*lev*100; usdt = pos*(pct/100)
    ...
    with right:
        ...
        st.dataframe(sim.style.format({"예상 수익률(%)":"{:.2f}","예상 PnL(USDT)":"{:.2f}","R":"{:.2f}"}), use_container_width=True)
